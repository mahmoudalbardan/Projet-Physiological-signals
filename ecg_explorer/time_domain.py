...
def hr_parameters(nni=None, rpeaks=None):
    ...
    n5 = np.nanpercentile(hr, 5)
    n50 = np.nanpercentile(hr, 50)
    n25 = np.nanpercentile(hr, 25)
    n75 = np.nanpercentile(hr, 75)
    n95 = np.nanpercentile(hr, 95)
    
    # Output
    args = (hr.mean(), hr.min(), hr.max(), hr.std(ddof=1),n5,n25,n50,n75,n95)
    names = ('hr_mean', 'hr_min', 'hr_max', 'hr_std','hr_p5','hr_p25','hr_p50','hr_p75','hr_p95')
    return biosppy.utils.ReturnTuple(args, names)
...
def sdnn_index(nni=None, rpeaks=None, full=True, duration=300, warn=True):
    ...
    segments, seg = utils.segmentation(nn,  full=full, duration=duration, warn=warn)
